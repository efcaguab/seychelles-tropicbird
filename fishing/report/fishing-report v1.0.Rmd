...
author: "Fernando Cagua --- 26 August 2016"
output:
  word_document:
    toc: no
  html_document:
...
:    *It is at the core of SIF's mission to ensure the protection of Aldabra's marine life. Using historic landing data from the Aldabra subsistence fishery and previously published species level extinction risk information, I identified five species whose local populations are at very high risk of decline even from low levels of fishing pressure, or that are globally threatened. These species were characterised by both a very high vulnerability (potentially large declines caused by fishing) and a very low resilience (slow population recoveries). Specifically fishing of the `r print_names(names2, "4 - Very high")` is best avoided, and individuals should released upon capture. Furthermore, I identified a set of another `r sum(totals$n_sp[totals$Risk %in% c("2 - Moderate", "3 - High")])` species whose populations do not warrant full protection, but should be closely monitored for signs of decline. Limiting the landings of species at very high risk requires is not likely to substantially affect the obtention of high quality protein for Aldabra staff, but requires small modifications to the current fishing practices. The most important one being the avoidance of deep water fishing, where barotrauma induced mortality is likely and fisheries independent monitoring is not feasible.* 
...
- 4 - Very high risk: A species is at very high risk if it has both a very low resilience *and* a very high vulnerability (>80), *or* has been listed by the IUCN as globally vulnerable or endangered
...
- 2 - Moderate risk: A species is at moderate risk if does not qualify as high or very high risk and has has either a very low resilience pr a very high vulnerability (>80), or a low resilience and high vulnerability (>60 and <80), *or* has been listed by the IUCN as near threatened.
